...
const store = new Store({
  name: 'dexpad',
  defaults: { schemaVersion: CURRENT_SCHEMA_VERSION, startup: false, wallpaperMode: true, cards: [], connections: [] }
});
...
function currentState() {
  return { schemaVersion: CURRENT_SCHEMA_VERSION, startup: Boolean(store.get('startup')), wallpaperMode: typeof store.get('wallpaperMode') === 'boolean' ? store.get('wallpaperMode') : true, cards: getCards(), connections: getConnections() };
}
...
function saveWorkspace(rawCards, rawConnections, excludeSender = null) {
  const operation = saveQueue.then(() => {
    const normalizedCards = Array.isArray(rawCards) ? rawCards.map((c, i) => normalizeCard(c, i)).filter(Boolean) : [];
...
function destroyDesktopWindow(){
  if(attachTimer){clearTimeout(attachTimer);attachTimer=null;} attachBusy=false;attachTries=0;
  if(!desktopWin||desktopWin.isDestroyed())return;
  if(wallpaperApi){try{wallpaperApi.detachFromDesktop(desktopWin);}catch(_){} }
  desktopWin.destroy(); desktopWin=null;
}
function refreshWallpaper(){
  ...
  publishState();attachWallpaper();return true;
}
function startExplorerWatch(){
  ...
  explorerWatchTimer=setInterval(()=>{
    ...
    if(desktopWin&&!desktopWin.isDestroyed()&&wallpaperApi&&!wallpaperApi.isWindowAttached(desktopWin)){console.warn('[DexPad] Desktop attachment lost. Re-attaching…');attachWallpaper();}
  },4000);
}
function setWallpaperMode(enabled){
  store.set('wallpaperMode',enabled);
  if(enabled){if(!desktopWin||desktopWin.isDestroyed())createDesktopWindow();if(workspaceWin&&!workspaceWin.isDestroyed())workspaceWin.hide();}
  else{destroyDesktopWindow();showControlWindow();}
  publishState();rebuildTray();
}
