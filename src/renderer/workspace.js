...
function deleteIds(ids){if(WALLPAPER||!ids?.size)return;recordHistory(true);cards=cards.filter(c=>!ids.has(c.id));connections=connections.filter(c=>!ids.has(c.a)&&!ids.has(c.b));ids.forEach(id=>selectedIds.delete(id));reconcile();flushSave()}
